...
class EcranPrincipal(Ecran):
    def __init__(self, graphique: GraphiqueInterface, gestionnaire_etat_ecran: GestionnaireEtatEcran, sample_rate=44100):
        ...
        self.notes_clavier = {
            'a': 'C4', 'z': 'D4', 'e': 'E4', 'r': 'F4', 't': 'G4', 'y': 'A4', 'u': 'B4',
            'w': 'C5', 'x': 'D5', 'c': 'E5', 'v': 'F5', 'b': 'G5', 'n': 'A5', ',': 'B5',
            'é': 'C#4', '"': 'D#4', '(': 'F#4', '-': 'G#4', 'è': 'A#4', 's': 'C#5', 
            'd': 'D#5', 'g': 'F#5', 'h': 'G#5', 'j': 'A#5',
        }
    ...
    def initialiser_interface(self):
        ...
        frame_high_left = self.graphique.creer_frame(frame_high, bg='#cecece')
        frame_high_right = self.graphique.creer_frame(frame_high, bg='#cecece')
        ...
        self.boutonRadio1 = self.graphique.creer_radiobutton(frame=frame_high_right, variable=choix, value=60, label="Lent", command=lambda: update_fonctionRand(choix.get()))
        self.boutonRadio1.pack(padx=5, pady=5, side="left")
        self.boutonRadio2 = self.graphique.creer_radiobutton(frame=frame_high_right, variable=choix, value=90, label="Normal", command=lambda: update_fonctionRand(choix.get()))
        self.boutonRadio2.pack(padx=5, pady=5, side="left")
        self.boutonRadio3 = self.graphique.creer_radiobutton(frame=frame_high_right, variable=choix, value=120, label="Rapide", command=lambda: update_fonctionRand(choix.get()))
        self.boutonRadio3.pack(padx=5, pady=5, side="left")
        self.boutonRadio4 = self.graphique.creer_radiobutton(frame=frame_high_right, variable=choix, value="Aléatoire", label="Aléatoire", command=lambda: update_fonctionRand(choix.get()))
        self.boutonRadio4.pack(padx=5, pady=5, side="left")



        self.boutonStop = self.graphique.creer_button(frame=frame_high_right, fonction=lambda: self.stop_music(), label="STOP MUSIC")
        ...
        frame_high_left.pack(side="left", fill="both", expand=True)
        frame_high_right.pack(side="right", fill="both", expand=True)
    ...
    def read_random_sequence(self, tempo):
        print("tempo", tempo)
        if tempo != None :
            mp.generate_random_sequence(self, 25, "test.txt", tempo)
